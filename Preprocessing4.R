...
columns_to_drop <- c('livingSpaceRange', 'street', 'description', 'facilities', 
                     'geo_krs', 'geo_plz', 'scoutId', 'regio1', 'telekomUploadSpeed',
                      'regio3', 'geo_bln', 'date', 'houseNumber', 'streetPlain',"baseRentRange","serviceCharge","pricetrend")
...
print(missing_values(rental_data_copy, 10))
...
# For rest features we just simply remove NA
print(table(rental_data_copy$floor))
print(table(rental_data_copy$firingTypes))
print(table(rental_data_copy$telekomTvOffer))
rental_data_copy$floor[is.na(rental_data_copy$floor)]<- median(rental_data_copy$floor, na.rm = TRUE)
rental_data_copy$firingTypes[is.na(rental_data_copy$firingTypes)]<- get_mode(rental_data_copy$firingTypes, na.rm = TRUE)
rental_data_copy$telekomTvOffer[is.na(rental_data_copy$telekomTvOffer)]<- get_mode(rental_data_copy$telekomTvOffer, na.rm = TRUE)
missing_values(rental_data_copy, ncol(rental_data_copy))
### Outliers ###
...
# Calculate new columns to eliminate multicolinerity
